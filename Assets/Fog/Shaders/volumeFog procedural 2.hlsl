...
void volumeFog_float(
    ...
    for (int i = 0; i < Samples ; i++) {
        ...
        //noise = sampleNoises(p * Noise2DScale);

        //noise = SAMPLE_TEXTURE3D(Noise3DTexture, sampler_linear_repeat, p * NoiseScale).r - 0.5;
        //noise = Noise3DTexture.Sample(sampler_linear_repeat, p * NoiseScale).r - 0.5;
        noise = Noise3DTexture.SampleLevel(sampler_linear_repeat, p * Noise2DScale, 0).r - 0.5;
        topBottomFade = saturate(yDistance* 1.25)  * saturate((OverallHeight - yDistance)* 1.25);

        // ______________________
        // --- NEW: DISTANCE DENSITY CALCULATIONS ---
        float currentDist = length(vectorToAdd);

        // Ramps from 1.0 near the camera up to 4.0 at MaxDistance (change 3.0 to make it even thicker!)
        float distRatio = saturate(currentDist / MaxDistance);
        float distanceMultiplier = 1.0 + (distRatio * 3.0);

        // (Optional) Keeps the immediate 5 units in front of the camera clear of heavy fog
        float nearCameraFade = smoothstep(0.0, 5.0, currentDist);


                // Sample the 3D Mask Texture based on world boundaries
        float areaMask = Mask3DTexture.SampleLevel(sampler_linear_repeat, p * Noise2DScale, 0).r;

        // Only accumulate fog where the mask texture is white
        Fog += saturate((noise - Threshold) * Multiplier * areaMask);


        // 3. Multiply distanceMultiplier and nearCameraFade into the final accumulation
        //Fog += saturate((noise - Threshold) * Multiplier * topBottomFade * distanceMultiplier * nearCameraFade);
        // ______________________
