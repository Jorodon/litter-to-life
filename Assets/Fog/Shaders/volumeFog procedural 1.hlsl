...
void volumeFog_float(
    ...
    Samples = distanceToStart < MeshDistance ? min(Samples, 90) : 0; // 50 will be the maximum samples, you can change this value if needed
    ...
    for (int i = 0; i < Samples ; i++) {
        ...
        float distanceMultiplier = 1.0 + (distRatio * DistanceMultiplier);

        float nearCameraFade = smoothstep(0.0, 3.0, currentDist);
        float farCameraFade = 1.0 - smoothstep(80.0, 200.0, currentDist);

        Fog += saturate((noise - Threshold) * Multiplier * topBottomFade * distanceMultiplier * nearCameraFade * farCameraFade);
    }
